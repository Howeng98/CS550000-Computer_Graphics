...
void updateLight() {
	...
	// Update each lightsource parameters
...
void RenderScene(int per_vertex_or_per_pixel) {	
	...
	glUniform1i(is_pixel_vertex, per_vertex_or_per_pixel);
	for (int i = 0; i < models[cur_idx].shapes.size(); i++)
	{
		glBindVertexArray(models[cur_idx].shapes[i].vao);

		// Calculate the offset from the txeture coordinate
		vector<Vector2> offsets;
		offsets.push_back(Vector2(0.0, 0.0));
		offsets.push_back(Vector2(0.0, 0.75));
		offsets.push_back(Vector2(0.0, 0.5));  
		offsets.push_back(Vector2(0.0, 0.25)); 

		offsets.push_back(Vector2(0.5, 0.0));  
		offsets.push_back(Vector2(0.5, 0.75));  
		offsets.push_back(Vector2(0.5, 0.5)); 
		...
		// Toggle magnification and minification between nearest and linear.
		if (magNearest) 
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_ESCAPE:
			...
			break;
		case GLFW_KEY_Z:
			cur_idx = (cur_idx == 0) ? 6 : cur_idx - 1;
			cout << "Switch to previous model" << endl;
			break;
		case GLFW_KEY_X:
			cur_idx = (cur_idx == 6) ? 0 : cur_idx + 1;
		...
		case GLFW_KEY_E:
			...
			cout << "ViewEye mode" << endl;
			break;
		case GLFW_KEY_C:
			cur_trans_mode = ViewCenter;
			cout << "ViewCenter mode" << endl;
			break;
		case GLFW_KEY_U:
			cur_trans_mode = ViewUp;
			cout << "ViewUp mode" << endl;
			break;
		case GLFW_KEY_L:
			LightIdx = 0;
		...
		case GLFW_KEY_K:
			...
			cout << "LightEdit mode" << endl;
			break;
		case GLFW_KEY_J:
			cur_trans_mode = ShininessEdit;
			cout << "ShininesEdit mode" << endl;
			break;
		case GLFW_KEY_G:
			magNearest = !magNearest;
			if (magNearest == true){
				cout << "Magnification with nearest" << endl;
			}
			else {
				cout << "Magnification with linear" << endl;
			}
			break;
		case GLFW_KEY_B:
			minNearest = !minNearest;
			if (minNearest == true) {
				cout << "Minification with nearest" << endl;
			}
			else {
				cout << "Minification with linear_mipmap_linear" << endl;
			}
			break;
		case GLFW_KEY_RIGHT:
			models[cur_idx].cur_eye_offset_idx = (models[cur_idx].cur_eye_offset_idx + 1) % models[cur_idx].max_eye_offset;
			cout << "Current_Eye_ID: " << models[cur_idx].cur_eye_offset_idx + 1 << endl;
			break;
		case GLFW_KEY_LEFT:
		...
		if (starting_press_x < 0 || starting_press_y < 0) {
			...
			switch (cur_trans_mode){
				...
				case LightEdit:
					...
					LightInfo[LightIdx].Position.x += 0.005 * diff_x;
					LightInfo[LightIdx].Position.y -= 0.005 * diff_y;
					break;
			}
		}
	}
...
void LoadTexturedModels(string model_path)
{
	...
	for (int i = 0; i < materials.size(); i++)
	{
		...
		// check whether the filename is EyeDh
...
void setUniformVariables()
{
	...
	// Light, pixel_vertex, and shininess 
	is_pixel_vertex = glGetUniformLocation(program, "is_pixel_vertex");
	...
	// Directional light
	LightSource_Object[0].Position = glGetUniformLocation(program, "light[0].Position");
	LightSource_Object[0].Ambient = glGetUniformLocation(program, "light[0].Ambient");
	LightSource_Object[0].Diffuse = glGetUniformLocation(program, "light[0].Diffuse");
	LightSource_Object[0].Specular = glGetUniformLocation(program, "light[0].Specular");
	LightSource_Object[0].Direction = glGetUniformLocation(program, "light[0].spotDirection");
	LightSource_Object[0].spotCutoff = glGetUniformLocation(program, "light[0].spotCutoff");
	LightSource_Object[0].spotExponent = glGetUniformLocation(program, "light[0].spotExponent");
	LightSource_Object[0].constantAttenuation = glGetUniformLocation(program, "light[0].constantAttenuation");
	LightSource_Object[0].linearAttenuation = glGetUniformLocation(program, "light[0].linearAttenuation");
	LightSource_Object[0].quadraticAttenuation = glGetUniformLocation(program, "light[0].quadraticAttenuation");

	// Point light
	LightSource_Object[1].Position = glGetUniformLocation(program, "light[1].Position");
	LightSource_Object[1].Ambient = glGetUniformLocation(program, "light[1].Ambient");
	LightSource_Object[1].Diffuse = glGetUniformLocation(program, "light[1].Diffuse");
	LightSource_Object[1].Specular = glGetUniformLocation(program, "light[1].Specular");
	LightSource_Object[1].Direction = glGetUniformLocation(program, "light[1].spotDirection");
	LightSource_Object[1].spotCutoff = glGetUniformLocation(program, "light[1].spotCutoff");
	LightSource_Object[1].spotExponent = glGetUniformLocation(program, "light[1].spotExponent");
	LightSource_Object[1].constantAttenuation = glGetUniformLocation(program, "light[1].constantAttenuation");
	LightSource_Object[1].linearAttenuation = glGetUniformLocation(program, "light[1].linearAttenuation");
	LightSource_Object[1].quadraticAttenuation = glGetUniformLocation(program, "light[1].quadraticAttenuation");

	// Spot light
	LightSource_Object[2].Position = glGetUniformLocation(program, "light[2].Position");
	LightSource_Object[2].Ambient = glGetUniformLocation(program, "light[2].Ambient");
	LightSource_Object[2].Diffuse = glGetUniformLocation(program, "light[2].Diffuse");
	LightSource_Object[2].Specular = glGetUniformLocation(program, "light[2].Specular");
	LightSource_Object[2].Direction = glGetUniformLocation(program, "light[2].spotDirection");
	LightSource_Object[2].spotCutoff = glGetUniformLocation(program, "light[2].spotCutoff");
	LightSource_Object[2].spotExponent = glGetUniformLocation(program, "light[2].spotExponent");
	LightSource_Object[2].constantAttenuation = glGetUniformLocation(program, "light[2].constantAttenuation");
	LightSource_Object[2].linearAttenuation = glGetUniformLocation(program, "light[2].linearAttenuation");
	LightSource_Object[2].quadraticAttenuation = glGetUniformLocation(program, "light[2].quadraticAttenuation");
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "110062401 HW3", NULL, NULL);
    if (window == NULL)
    {
